...
const NAV_ITEMS = [
  ...
  ["standardisation", "Response Standardisation"], ["validation", "Validation"], ["export", "Export"],
];

const EMPTY_WORKFLOW = [
  ["datasets", "Datasets"], ["mapping", "Mapping"], ["standardisation", "Response Standardisation"],
  ["validation", "Validation"], ["export", "Export"],
].map(([page, label], index) => ({ page, label, status: index === 0 ? "current" : "upcoming" }));

function WorkspaceLayout({ activePage, activeRun, runs, busy, notice, onDismissNotice, onNavigate, onSelectRun, children }) {
  const workflow = activeRun?.workflowProgress?.steps || EMPTY_WORKFLOW;
  return (
    <div className="app-shell">
      ...
      <div className="workspace-grid">
        <aside className="sidebar" aria-label="Harmonisation workflow">
          ...
          <div className="workflow-status"><p className="eyebrow">Workflow</p>
            {workflow.map((step) => <div className={`workflow-step ${step.status === "upcoming" ? "pending" : step.status}`} key={step.page}><span aria-hidden="true" />{step.label}</div>)}
          </div>
      ...
      <footer className="workspace-footer">
        <span>Tasmanian Leaders &middot; Data Harmonisation Platform</span>
        <span>Internal workspace. Identifiers are pseudonymised before export.</span>
      </footer>
    </div>
  );
}
